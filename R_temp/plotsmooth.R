...
#' @param maxsmooth The maximum f value of the loess function to visualize (defaults to 0.22)
...
plotsmooth <- function(all.specs, minsmooth = 0.02, maxsmooth = 0.22, curves = 5, specnum = 0){
...
legnames <- paste("Loess f = ",legnames, sep="")
...
for (i in 1:nplots){
  ...
  lowess(all.specs[, 1] ,all.specs [, i+1], f = minsmooth)$y + 5
  
  plotdata[, ((i-1)*curves)+curves] <-
  lowess(all.specs[, 1] ,all.specs [, i+1], f = maxsmooth)$y + ((curves-1)*5)
  
  for (j in 1:(curves-3)){
    plotdata[, ((i-1)*curves)+2+j] <- 
    lowess(all.specs[, 1] ,all.specs [, i+1], f = (minsmooth+(inc*j)))$y + (10 + ((j-1)*5))
    }
}
