...
# 2. implement switch, match.arg
 
procspec <- function(Y, f=NULL, sm=FALSE, spar = .65, span = .15, method="spline") {
...
# NOTES ON BETTER WAY TO DO THIS FUNCTION:
	# proc = c('smooth', 'normalize')
	# method = c('loess', 'spline', 'max', 'min')
	# proc <- match.arg(proc, c('smooth', 'normalize')
	# method <- match.arg(method, c('loess', 'spline', 'maximum', 'minimum', 'stretch'))
