...
govardovskii <- function(peaksense, range = c(300,700), lambdacut = NULL, Bmid = NULL, 
		             oiltype = NULL, beta = TRUE) {

if (!is.null(lambdacut)){
 if (is.null(Bmid) & is.null(oiltype)) stop ("Bmid or oiltype must be included when including a lambdacut vector")
 if (length(lambdacut) != length(peaksense)) stop ("Length of lambdacut must be same as peaksense")
}  
