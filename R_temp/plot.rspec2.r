...
if (type=='stack') {
  ...
  specs2 <- specs
  y <- apply(specs2, 2, max) 
  ym <- cumsum(y)
  ymins <- c(0, ym[-length(ym)])
  plot(specs2[, 1]~wl, type='l', xlim = xlim, ylim = c(0, sum(y)), 
       xlab = 'Wavelength (nm)', ylab = 'Cumulative reflectance (arb. units)', 
       col = col[1], ...)
  if (ncol(specs)>1) {
    for (i in 2:ncol(specs)) 
      lines((specs2[, i] + ymins[i])~wl, col = col[i], ...)
    }
#  axis(2, at=cumsum(ym)-cumsum(ym)[1], substr(names(specs2), 1, 5), las=1, cex.axis=.5)
  yend <- tail(specs2, 1)
  yloc <- ymins + yend
  text(x = tail(wl, 1), y = yloc, names(specs), cex=.5, pos=4)
#  abline(h=ymins)
}
