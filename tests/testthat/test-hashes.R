...
test_that("coldist", {
  ...
  # Output
  expect_identical(digest::sha1(coldist(colspace(
  ...
  expect_identical(digest::sha1(
    coldist(
      colspace(vismodel(
        flowers, visual = "canis", achromatic = "all"
      )),
      n = c(1, 2),
      achromatic = TRUE,
...
test_that("coldist_nolinux", {
  ...
  expect_identical(digest::sha1(coldist(
...
test_that("bootcoldist", {
  ...
  expect_identical(digest::sha1(c(bcd.sic[, 1], bcd.sic[, 4])),
...
test_that("special_colspace", {
  ...
  expect_identical(digest::sha1(colspace(
    vismodel(flowers, visual = "canis", achromatic = "all")
  ), digits = 3),
  "41e6dafe465f01a883abd395c00be168aec45b2d") # dispace
  expect_identical(digest::sha1(colspace(
    vismodel(flowers, visual = "apis", achromatic = "l")
  ), digits = 3),
  "f327404a1e417cef90d5c79a5cfd6ecb8c494de4") # trispace
  expect_identical(digest::sha1(colspace(
    vismodel(flowers, visual = "bluetit", achromatic = "ch.dc")
  ), digits = 3),
  "d6d882283d9f005f436066b7da0d5e9a4c4b1e15") # tcs
})

test_that("special_colspace", {
  expect_identical(digest::sha1(colspace(
    vismodel(flowers, visual = "musca", achro = "md.r1"), space = "categorical"
  ), digits = 4),
  "b20853b3e52a60f2dd17b418a48b681d7f49e7d1") # categorical
  expect_identical(digest::sha1(colspace(
  ...
  digits = 4),
  "a1fdd24e315413825c94d4caf1164b8be57c8156") # hexagon
  expect_identical(digest::sha1(colspace(
    vismodel(flowers, visual = "cie10"), space = "ciexyz"
  ), digits = 4),
  "99684793a0db286562bff697354496ac3ef0abfb") # ciexyz
  expect_identical(digest::sha1(colspace(
    vismodel(flowers, visual = "cie10"), space = "cielab"
  ), digits = 4),
  "55961f7e22403fba0c0c658868918722befb5f2c") # cielab
  expect_identical(digest::sha1(colspace(
    vismodel(flowers, visual = "cie10"), space = "cielch"
  ), digits = 4),
  "e0ad250b695e97c9ffb53c0303f19d908e33d033") # cielch
...
test_that("processing & general", {
  # Sensdata
  expect_known_hash(expect_silent(sensdata(
    illum = "all",
    bkg = "all",
    trans = "all"
  )),
  ...
  # Peakshape
  expect_known_hash(expect_silent(peakshape(flowers, absolute.min = TRUE)),
                    "7fbaba1738")
  
  # Simulate
  # Ideal
  expect_known_hash(digest::sha1(summary(simulate_spec(ylim = c(
    0, 50
  ))), digits = 4),
  "88a30943a99d1ea14bc42ac4e4f4d54d")
  # Sigmoidd low-high
  expect_known_hash(digest::sha1(summary(simulate_spec(wl_inflect = 550)), digits = 4),
                    "8e3891ab1c6cf8a10bcab6410fa2f4a0")
  # Sigmoid high-low
  expect_known_hash(digest::sha1(summary(
    simulate_spec(wl_inflect = 550, ylim = c(100, 0))
  ), digits = 4),
  "2b9191b2b7107e2903702b77a80c432d")
  # Gaussian
  expect_known_hash(digest::sha1(summary(simulate_spec(wl_peak = 400)), digits = 4),
                    "9e86606691410fa965031f7b6b180c48")
  
  # Merge
  teal1 <- teal[, c(1, 3:5)]
  teal2 <- teal[, c(1, 2, 6:12)]
  expect_known_hash(expect_silent(merge(teal1, teal2, by = "wl")),
                    "02df3eedf3")
  
  # Subset
  vis.sicalis <- vismodel(sicalis)
  tcs.sicalis <- colspace(vis.sicalis, space = "tcs")
  expect_identical(digest::sha1(subset(vis.sicalis, "C"), digits = 4),
                   "93ae671c250d2d4f0f5dcf9e714eb497d8baf74f")
  expect_identical(digest::sha1(subset(sicalis, "T", invert = TRUE), digits = 4),
                   "332a97ed1c25045b70d871a8686e268d09cefd76")
  
  # Summary
  expect_known_hash(expect_silent(summary(teal)),
                    "c64e1fd403")
  expect_known_hash(expect_silent(summary(sicalis)),
                    "66129550f3")
})
...
test_that("images", {
  ...
  expect_identical(digest::sha1(summary(papilio), digits = 4),
                   "aa1c46d4796c523f51c4e959ac90a692dd8ecfe4")
  expect_identical(digest::sha1(summary(snakes), digits = 4),
...
test_that("vismodel", {
  ...
  expect_identical(digest::sha1(vismodel(flowers, visual = "cie10"), digits = 4),
