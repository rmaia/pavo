...
#' 
#' @param labels plot verticy labels (or category labels, when \code{space = 'categorical'})? 
...
#'  outline when \code{space = 'ciexyz'} if \code{mono = TRUE} (defaults to 1).  
#' @param out.lcol line colour for plot outline, or the colour of the monochromatic loci
#'  outline in \code{space = 'ciexyz'} if \code{mono = TRUE} (defaults to black). 
#' @param out.lty line type for plot outline, or the line type for the monochromatic loci
#'  outline in \code{space = 'ciexyz'} if \code{mono = TRUE} (defaults to 1).
#' @param tick.loc a numeric vector specifying the location of tick marks on x & y axes,
#'  only when \code{space = 'di'}
#' @param mono plot the monochromatic loci (the 'horseshoe') when \code{space = 'ciexyz'}? 
#'  Defaults to \code{TRUE}.   
#' @param sectors plot the bee-hue sector dividers when \code{space = 'hexagon'}? 
#' Options are:
#'    \itemize{ 
...
#'        }
#'        
#' @param col.sec line colour of bee-hue sector dividers when \code{space = 'hexagon'}. 
#'  Defaults to \code{'grey'}
#' @param new should a new 3D plot be called, when \code{space = 'tcs'} (defaults to \code{FALSE})?
#' @param hspin if \code{TRUE}, the graphic will spin horizontally (around the 'z' axis, when \code{space = 'tcs'})
#'  (defaults to \code{FALSE})
#' @param vspin if \code{TRUE}, the graphic will spin vertically (around the 'x' axis, when \code{space = 'tcs'})
#'  (defaults to \code{FALSE})
#' @param floor if \code{TRUE}, a reference xy plane is plotted under the tetrahedron, when \code{space = 'tcs'} 
#'  (defaults to \code{TRUE})
#' @param grid if \code{TRUE}, connects the polygon outlining the volume occupied by points, when \code{space = 'tcs'}
#'  (defaults to \code{TRUE})
#' @param grid.alpha transparecny of the volume polygon grid lines, when \code{space = 'tcs'}
#' @param fill if \code{TRUE}, fills the volume occupied by points, when \code{space = 'tcs'} 
...
#'  \code{points.colspace} adds points to the plot. When \code{space = 'tcs'}, 
