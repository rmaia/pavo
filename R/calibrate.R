...
#' @param assign_n_cols do you want to manually specify the number of discrete colour
...
calibrate <- function(image, scale_length = NULL, assign_n_cols = FALSE) {

  ## Checks
  multi_image <- inherits(image, "list") # Single or multiple images?
  if(is.null(scale_length) && assign_n_cols == FALSE)
    stop("No options selected.")

  if (isTRUE(multi_image)) { # Multiple images
    ## Assign k ##
    if (isTRUE(assign_n_cols)) {
