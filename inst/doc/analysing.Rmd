...
`pavo` now allows for the combined analysis of spectral data (as above) and spatial data (as below). Note that 'spatial' data of course include images, but may also include spatially-sampled _spectra_, as explored in the examples below. Thus while much of the below deals with image-based processing and analysis, images are not necessary for many spatial analyses and indeed may perform worse than spectrally-based analyses in some cases. 
...
This approach has clearly identified four distinct colour patches, as we simulated. To be able to feed this information directly in to `adjacent()` we need to arrange the now-classified spectral data into a xyz data-image matrix, in which `x` and `y` coordinates specify the location of the sample in our scene, and the z values at each location specify the colour-class, or group, into which each sample has been classified (an integer). Conveniently enough, that particular clustering method offers the data's classification as output, so we can simply rearrange it into our original 10 x 10 sampling matrix before passing it to `adjacent()`.
