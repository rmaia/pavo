...
question in evolutionary biology for centuries
\citep{darwin:1859,darwin:1896,Poulton:1890,Bennett:2007hm}. Visual communication has provided an
unique opportunity to investigate aspects of natural \citep{Chittka:1992} and sexual selection
(REF), and how these interact \citep{,Kemp:2009}  (REFS- SUGGESTIONS?). It it also an ideal  system
for truly integrative biological research, from the optical processes generating color (REF),
hormonal and genetic mechanisms regulating this expression (REF), physiological processes involved
in preceiving the signal (REF), and its adaptive and evolutionary patterns
