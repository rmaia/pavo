...
Fechner's law (see Figure \ref{figure:logplot}).
...
% IDEAS ON THIS PARAGRAPH RAFAEL? I KINDA THREW IT TOGETHER..

Body colouration may play an important role in reproductive isolation and, hence, speciation (e.g.,
see \cite{Seehausen 1999}, \cite{Ritchie?}).  African starlings (Sturnidae) are a speciose clade of
brilliant iridescent birds whose colours range from ultraviolet to red \cite{REF?}. Recent
phylogenetic evidence indicates a very recent divergence for two species \emph{Lamprotornis
chloropterus} and \emph{L. elisabeth} compared to the node containing their sister species \emph{L.
acuticaudus} (\cite{rubenstein}). Given the lability of iridescent plumages \cite{} and potential to
produce colours across the bird-visible range of wavelengths \cite{}, one might predict more
divergent colours between distantly related species. However, these three species have numerous
discrete plumage patches that vary in colouration both within species as well as within individuals.
By analyzing these patches within an avian colourspace, we can better understand the physiologically
relevant divergence in plumage colour among these focal species while also demonstrating some of the 
capabilities of \pavo{} in analyzing spectral data.
