...
Visual communication has provided an unique opportunity to investigate aspects of natural
...
objective quantification of the colour reflected by surfaces, as a first approximation of an
...
standardized, and software for analyses are limited either in the methods they implement, the types
of data they import and process, or the platforms in which they are available. Moreover, many are
proprietary and/or closed, making customization impossible \citep[for a review of available
...
CRAIC). In addition, previously compiled data frames can be imported to R and converted to
...
their colours, and compared both to their sister species, the sharp-tailed starling (\emph{L.
acuticaudus}).
...
\caption{Plots of mean smoothed spectra for different body patches (belly, left; wing, right) in three
...
(The calculation of both chromatic and achromatic distances of these 8646 pairs of colours took about
0.9 seconds in a MacBook Pro running R version 2.15.2). We can see from Figure \ref{figure:distplot}
that colour disparity between the closely related species is similar to that between them and the
more distantly related sister species  \emph{L. acuticaudus}. Using a value of 1 as a threshold for
Just Noticeable Differences (JNDs, Figure \ref{figure:distplot} dashed lines) nearly all plumage
patch comparisons yield colours that are discernible to the starling visual system, both within the
\emph{L. chloropterus-L. elisabeth} subclade as well as compared to the \emph{L. acuticaudus}
outgroup. Given the high satuturation of iridescent colours, small differences in the spectral shape
can yield noticeably different colours. This becomes clear when comparing the results for the most
and least contrasting body patches (``belly'' and ``wing'', respectively; Figure
\ref{figure:distplot}) to their spectral curves (Figure \ref{figure:specsplot}).
...
# Plot mean +/- SD of colour distances by patch
