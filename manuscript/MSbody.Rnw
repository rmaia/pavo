...
\textcolor{red}{ADD CITATION TO AVICOL, TETRACOLORSPACE AND REVIEWED IN MONTGOMERIE)}, they are
...
\section{Worked example: Color divergence in glossy starlings (Sturnidae)}
...
\code{summary.rspec} and \code{vismodel} functions, respectively. The former characterizes color
...
or between color patches and a given background). This argument employs regular expressions
...
comparisons yield discernible colors, both within the \emph{L. chloropterus \textendash{} L.
...
\citep{Andersson:2006} for discussion). As many as 23 objective colorimetric \textcolor{red}{RM
...
\pavo{} implements color analysis for reflectance data based on both spectral properties and visual
...
and validate the use of different colorimetrics \citep{Butler:2011ju} and determine the influence
...
allow for any number of photoreceptor to be included, but color distances can so far only be
calculated for tetrachromats. Implementing extensions of the \cite{Vorobyev:1998} model would allow
\pavo{} to calculate color distances for the visual systems of organisms such as butterflies (e.g.
...
initial data input to final analysis. It therefore permits the inclusion of color analysis from
spectral data in open and reproductible workflows, which can thus be directly automated and adjusted
...
\citealt{Siebeck:2001}) as they become available, providing a repository of spectral and color data
...
variables (`objective colorimetrics') and their specific or universalrelevances to signal production
or signal perception mechanisms, see \cite{Montgomerie:2006} and  \cite{Andersson:2006}. For
visualmodels based on photon catches and receptor noise, see  \cite{Vorobyev:1998} and
...
color distances (in units of just noticeable differences, JNDs)  by patch (y-axis) for three pairs
...
Plot of mean smoothed spectra for the belly body patch. Line colors indicate  species (green:
