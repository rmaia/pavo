...
three glossy starling species. The data used for this example can be found in the Supplemental
Information.
...
are located in the folder \code{'/Desktop/glossystarlings'} (by default, the current working
directory and the ``.txt'' extension are used). We measured reflectance spectra from 11 plumage
patches (see Figure \ref{figure:distplot} for list) of four males from museum specimens of each of
the three species. Three measurements from different locations within each patch were collected. We
used \pavo{} to load these 396 \textcolor{red}{(RM: Is this number right?  my math may be wrong but
i find it should add to 132)} raw spectral data files (using the \code{getspec} function), then used
the \code{aggspec} and \code{procspec} functions to average the spectra within patches (as
determined by the \code{by} argument) and remove electrical noise arising from the spectrometer
(using local polynomial regression fitting, or loess), respectively, with the following annotated
lines of code:
...
the mean reflectance curves for each body part from each species, as can be seen in the example
below for the ``belly'' body patch (Figure \ref{figure:distplot}B):
...
attributes from the spectral shape of reflectance curve \textcolor{red}{(RM: Chad check wording)},
whereas the latter takes into account avian visual sensitivities (sensory phenotype). \pavo{} allows
for user-defined cone sensitivities, but also incorporates available data on the from several taxa,
including the European starling (\emph{Sturnus vulgaris}, \code{visual='star'}), which we
incorporated here \citep{Hart:1998}. 
...
plumage patches in the \code{tcs} object \textcolor{red}{RM: But tcs isn't used in coldist)}, and
can be further subset (using  the \code{subset} argument) to focus on comparisons of interest (e.g.,
colour difference between homologous patches of two species, or between color patches and a given
background). This argument employs regular expressions functionality, and is thus allows for partial
matching of column \textcolor{red}{(RM: Row?)} names from the results. For example:
...
it was before)} have accumulated similar levels of color disparity among them as they have to their
sister species, \emph{L. acuticaudus}. Considering a value of 1 as a threshold for Just Noticeable
...
compared to the \emph{L. acuticaudus} outgroup. \textcolor{red}{(RM: Moved the content of the last sentence to the next
section)}
...
boxplot(H1~spp, subset(df, patch=="belly"), xlab="Species", ylab="Hue (nm)", xaxt='n', lty=1, lwd=1.5, cex.lab=1.2, col=pal, staplewex=0.2)
