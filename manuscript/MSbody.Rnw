...
\citep{Montgomerie:2006}.
Description and formulae for the variables can be also be found in the help file, by typing
...
courtship displays and social competition \citep{Rubenstein:2009}. Here we exemplify some of
\pavo{}'s capabilities through a worked example comparing the colours of a monophyletic clade of
three glossy starling species, two of which have recently diverged and whose status as full
species is debated \textcolor{blue}{CE: does this make sense?} \citep{Gill:2012,Craig:1998,
Lovette:2007}. The data used for this example can be found in the Supplemental Information.
...
\textcolor{red}{RM: INCLUDE SOMEWHERE HERE ABOUT FILE NAMING?}
\textcolor{blue}{CE: How is this? (see below)}

The data consist of reflectance spectra (in avantes ``.ttt'' output format), which in this example
are located in a folder \code{'/Desktop/glossystarlings'} (by default, the current working directory
and the ``.txt'' extension are used). The file names indicate the species, bird ID,  sex, plumage
patch and measurement ID as follows: \code{LAAC.14423.MALE.belly-001.ttt}. We recommend that users
carefully consider and adhere to a rigorous file naming scheme to make subsetting and data
manipulation easier in \code{pavo} \citep[see][for a useful discussion]{Montgomerie:2006}. We
measured reflectance spectra from 11 plumage patches (see Figure \ref{figure:distplot} for list) of
four males from museum specimens of each of the three species. Three measurements from different
locations within each patch were collected. We used \pavo{} to load these 396 \textcolor{red}{(RM:
Is this number right?  my math may be wrong but i find it should add to 132)} \textcolor{blue}{(CE:
see line preceding this one.)} raw spectral data files (using the \code{getspec} function), then
used the \code{aggspec} function to average the spectra within patches (as determined by the
\code{by} argument) and the \code{procspec} function to remove electrical noise arising from the
spectrometer (using local polynomial regression fitting, or loess) with the following annotated
lines of code:
...
results of \code{coldist} give colour distances (chromatic , $\Delta S$, Figure
\ref{figure:distplot}; and achromatic, $\Delta L$, not shown) between all possible combinations of
...
expressions functionality, and is thus allows for partial matching of row names from the results.
For example:
...
\emph{L. elisabeth} \textcolor{red}{(RM: the intro to this section doesn't include the information
that these species are recently divergent, should we include it back? I think so, maybe briefer than
it was before)} \textcolor{blue}{(CE: I changed the paragraph introducing the example. See what you
think.)} have accumulated similar levels of color disparity among them as they have to their sister
species, \emph{L. acuticaudus}. Considering a value of 1 as a threshold for Just Noticeable
Differences (JNDs, Figure \ref{figure:distplot}A dashed lines) nearly all plumage patch comparisons
yield discernible colors, both within the \emph{L. chloropterus-L. elisabeth} subclade as well as
compared to the \emph{L. acuticaudus} outgroup. \textcolor{red}{(RM: Moved the content of the last
sentence to the next section)}
...
\textcolor{red}{RM: Maybe use the same colors for the boxplot as in the spectra?}
\textcolor{blue}{CE: Good call. I like the new version.}
...
A few areas of research will find \pavo{} particularly useful. For example, it will now be easier 
to compare and validate the use of different colorimetrics \cite{Butler:2011} and determine 
the influence of various assumptions in visual models \cite{Hanley:Inpress}. \pavo{} can also be used 
to investigate the information content of colour signals using receiver specific visual systems 
\cite{Pike:2011} and answer questions pertaining to signal production mechanisms (e.g. comparing 
the relative contribution of pigments and structural colours in various integuments; \cite{D'Alba:2012}).
While \pavo{} is already fairly comprehensive, many functions and reference data could (and will)
be implemented in the future. For example, we would like to increase the flexibility of the visual
modelling functions to allow for any number of photoreceptor to be included. This would allow 
\pavo{} to model the visual systems of organisms such as butterflies (e.g. six photoreceptors in 
the genus \emph{Papilio} ; \cite{Arikawa:2003}) and mantis shrimps (Order Stomapoda; 16 photoreceptors;  
\cite{Cronin:1993}). 

Furthermore, reference data for illuminants, backgrounds, photoreceptors, 
and other components of visual systems (e.g. ocular media; \cite({Siebeck:2001}) will 
continue to be added to updated versions of the package to develop a comprehensive yet 
flexible research tool. Researchers in relevant fields are encouraged to contact the corresponding 
author if they would like to contribute their data to this effort.
