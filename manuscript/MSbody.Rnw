...
with by scaling photon catch values by a large value, but given the objective of this transformation
...
\citep{Craig:1998,Lovette:2007}. We compared to what extent these two species have diverged in their
...
spp <- substr(names(specs.belly),1,4)[-1]  # get 4 first character form spec names (species labels)
...
S$,][]{Vorobyev:1998to}. We used relative cone abundances for the European starling
