...
popularity of portable spectrometers have made objective quantification of the spectral
...
reflectance data is needed. Output file types from spectrometer manufacturers are not
standardized, and software programs for analyses are limited either in the methods they implement,
...
function \code{getspec}, which currently supports data from a variety of software (Ocean Optics OOIBase 
and SpectraSuite files, Avantes AvaSpec, and
...
are located in the folder \code{'/Desktop/glossystarlings'}. We measured reflectance spectra from 11
plumage patches (see Figure \ref{figure:distplot} for list) of four males from museum specimens of
each of the three species. Three measurements from different locations within each patch were
collected. We used \pavo{} to load these 396 \textcolor{red}{(RM: Is this number right?  my math may
be wrong but i find it should add to 132)} raw spectral data files, average the spectra within
patches and remove electrical noise arising from the spectrometer (using local polynomial regression
fitting, or loess) with the following annotated lines of code:
...
\begin{acks} We would like to thank Jarrod D. Hadfield and Mary Caswell Stoddard for sharing code
...
FA9550-09-1-0159 (M.D.S.), the University of Akron (M.D.S., R.M., C.M.E.), NSERC Discovery grant
and  Equipment grant (S.M.D), and NSERC Graduate Scholarship (P.-P.B.). 
\end{acks}
