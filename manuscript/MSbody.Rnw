...
specs.belly <- specs[c(1, grep('belly', names(specs)))]  # subset of spectra from the belly patch
spp <- sapply(strsplit(names(specs.belly), '\\.'), "[", 1)[-1]  # get species info from spec names
aggplot(specs.belly, by = spp)  # average specs by species
@
...
\code{vismodel} function.

\subsubsection{Colour distances}

First, \code{relative = FALSE} was used to calculate raw \{\emph{usml}\} values, suitable for
calculating chromatic distances \citep[$\Delta S$,][]{Vorobyev:1998to}. We used relative cone
...
Weber fraction of approximately 0.05 \cite{Vorobyev:1998,Vorobyev:1998to}. The results of
\code{coldist} give color distances (chromatic , $\Delta S$, Figure \ref{figure:distplot};  and
achromatic, $\Delta L$, not shown) between all possible combinations of plumage patches in the
\code{tcs} object, and can be further subsetted to only analyse comparisons of interest (e.g.,
colour difference between homologous patches of two species). For example:

<<>>=
vm2 <- vismodel(specs, visual = 'star', relative = FALSE)
delta.S <- coldist(vm2, qcatch = 'fi', n1 = 1, n2 = 1.38, n3 = 3.34, n4 = 3.46, v = 0.1)
head(delta.S[grep('belly', delta.S$patch1),], 3)  # subset only comparisons with belly spectra
@

We can see from Figure \ref{figure:distplot} that the recently divergent \emph{L. chloropterus} and
\emph{L. elisabeth} have accumulated similar levels of color disparity among them as they have to
\emph{L. acuticaudus}. Considering a value of 1 as a threshold for Just Noticeable Differences
(JNDs, Figure \ref{figure:distplot} dashed lines) nearly all plumage patch comparisons yield colors
that are discernible to the starling visual system, both within the \emph{L. chloropterus-L.
elisabeth} subclade as well as compared to the \emph{L. acuticaudus} outgroup.
% This result is not surprising for iridescent colours like those analyzed here considering their 
% characteristically high saturation. NOTE - should we also state here how the analysis took 0.9
% seconds compared to the ~5 mins required for AviCol?
...
Second, for producing a tetrahedral colourspace plot, we used \code{relative = TRUE} in the
\code{vismodel} input to indicate that \{\emph{usml}\} values should be scaled to sum to unity for
...
it is more closely related (see Figures \ref{figure:specsplot} and dark grey circles in
\ref{figure:volplot}).
...
objects containing tetra colourspace data for each species, as output by the \code{tcs} function in
\pavo{}):

<<echo=T,fig=F,include=F>>=
voloverlap(tcs1b, tcs1c, plot = TRUE)
@
