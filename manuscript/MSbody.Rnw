...
these manually?} 
\textcolor{blue}{CE: here is what the output would be, but I'm not sure how we can 
make it look like real output. Thoughts?}
\textcolor{red}{RM: I think we'll just have to edit the tex file after sweaving, before submitting.}
...
determined by the argument \code{qcatch} (not used in the code below, hence the default \textemdash
$Q_i$ \textemdash is returned). It can also apply the von Kries transformation (normalizing by the
stimulus' background to account for receptor adaptation) through the \code{vonkries} argument (which
defaults to \code{FALSE}). All these settings are stored in the \code{vismodel} object and can be called using the \code{summary} function:
