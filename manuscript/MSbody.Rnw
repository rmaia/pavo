...
\title{\pavo{}: an R package for the perceptual analysis, visualization and organization of spectral colour data}
...
\item Animal colouration has been a powerful model for the study of a diverse range of topics in
...
\item Despite this growing interest, a unified framework for analyzing spectral data has not been
available. We introduce \pavo{}, an R package that facilitates the organization, visualization, and
analysis of spectral data in a cohesive framework. \pavo{} is highly flexible: it allows the user to
(a) organize and manipulate  data from a variety of sources, (b) visualize data with easily-to-use
tools, and (c) analyze data using visual system modeling for a broad range of taxa.
...
present suggestions (implemented in the package) for handling psychophysiological data, and an exact
solution for the calculation of colour volume overlap, thus expanding previously published
methodologies.
...
\item \pavo{} provides a unique environment capable of addressing complex sensory ecology questions
that previously required the use of multiple, sometimes restricted, software and programs.
...
evolutionary biology for centuries \citep{darwin:1859,darwin:1896, Poulton:1890,Bennett:2007hm}.
Visual communication has provided a unique opportunity to investigate aspects of natural
...
\citep{Endler:1993wm,Eaton:2003,Bennett:2007hm}. Over the last 20 years, the falling cost and rising
...
to intuitively import, explore, process and analyze spectral colour data under a variety of user-
defined models. We propose that combining these procedures under a coherent framework not only
...
CRAIC). In addition, previously compiled data frames can be imported into R and converted to
...
(hue, saturation and brightness) colour variables extracted from the spectral shape of the
...
\subsection{Volume Overlap and Fechner's law in the Tetrahedral colourspace Model}
...
models to the tetrahedral colourspace. For example, \cite{Endler:2005} recommend log-transforming
photon catches before defining the colourspace (by dividing the photon catch of each receptor by the
sum of all receptors, Figure \ref{figure:logplot}). This transformation makes the difference between
stimuli proportional to their magnitude \citep[i.e. following Fechner's law; see
also][]{Vorobyev:1998,Endler:1990}. \cite{Stoddard:2008} evaluated the consequences of this
transformation and noted dramatic and unpredictable effects in hue and chroma values. Though the
decrease in hue variance and an overall decrease in chroma values should be expected as a result of
the transformation, the unpredictable effects on hue discussed by \cite{Stoddard:2008} requires
explanation. As the authors noted \citep[see][Supplemental Information]{Stoddard:2008}, this is
largely a result of normalizing all input functions (reflectance, sensitivity, background and
illuminant) to integrals of 1, which yields cone stimulations ($Q_i$ for the cone class $i$) that
range from 0 to 1. Log-transforming these $Q_i$ values ($f_i = log(Q_i)$) results in negative
numbers, which if made relative by dividing by the sum ($f_{i\;relative} = f_i / \sum{f_i}$) results
in the greatest \textit{absolute} $f_i$ value (i.e. the ``most negative'' and thus smallest) being
the greatest $f_{i\;relative}$ value \textendash \; for example, $-0.5 > -5$, but $\frac{-0.5}{-5.5}
< \frac{-5}{-5.5}$. This reverses the relative ranking of photoreceptor classes, yielding erroneous
colourspace coordinates (Figure \ref{figure:logplot}, middle). \cite{Stoddard:2008} note that this
can be dealt with by scaling photon catch values by a large value, but given that the objective of
this transformation is to make stimuli distances proportional to their scale, arbitrary scale
transformations will result in undesirable effects. Instead, we propose the following transformation
for log-transformed photon catches ($fi$), which is adequate for making negative values relative to
each other and is implemented in \pavo{} when \code{vismodel} is executed with the option
\code{relative=TRUE}:
...
This calculation preserves measurement scale and yields hue values that are consistent with original
values, but with differences that are a linear function of their magnitude, as expected from
Fechner's law (Figure \ref{figure:logplot}, bottom). This transformation is only valid (and only
used by \code{vismodel}) when log-transformed quantum catches are negative values; in other cases
the transformations suggested by \cite{Endler:2005,Stoddard:2008} are valid.
...
Here we exemplify some of \pavo{}'s capabilities and workflow by comparing the colour patterns of a
monophyletic clade of three glossy starling species. Glossy starlings from the African clade have
bright and diverse iridescent colours likely used in sexual selection and social competition
...
\citep{Craig:1998,Lovette:2007}. We compared the extent to which these two species have diverged in
their colours and compared each to their most recently diverged sister species, the sharp-tailed
starling (\emph{L. acuticaudus}).
...
the electrical noise arising from the spectrometer (using local polynomial regression fitting, or
loess) from these 396 raw spectral data files using the following annotated lines of code in
\pavo{}:
...
these plots, as well as the results for the ``belly'' and ``wing'' body patches: (Figure
\ref{figure:distplot}):
...
To understand how these colours are perceived by birds, we used the \code{vismodel} function to
compare colours taking into account avian visual sensitivities (sensory phenotype). \pavo{}
incorporates available data on the retinal sensitivities of numerous taxa, including the European
...
\code{vismodel} function to measure colour attributes using the receptor noise and colourspace
models, respectively.
...
\citep{Vorobyev:1998,Vorobyev:1998to}. The results of \code{coldist} give colour distances (chromatic
, $\Delta S$, Figure \ref{figure:distplot};  and achromatic, $\Delta L$, not shown) between all
possible combinations of plumage patches in the \code{tcs} object, and can be further subset to
...
(``belly'' and ``wing'', respectively) to their spectral curves (see Figure \ref{figure:distplot}).
...
plot(d23[rs,1], ylim = c(0, 11), xlab = "", 
     ylab = expression(paste(Delta, "S (JNDs)")), xaxt = 'n', 
...
segments(seq(1,22,by=2)+pad, d23[rs, 1] - d23[rs, 2], seq(1,22,by=2)+pad, 
         d23[rs, 1] + d23[rs, 2])
points(d12[rs, 1]~I(seq(1,22,by=2)-pad), pch = 21, bg = 'black', cex=1)
points(d13[rs, 1]~seq(1,22,by=2), pch = 21, bg = 'grey', cex=1)
points(d23[rs,1]~I(seq(1,22,by=2)+pad), pch = 21, bg = 'white', cex=1)
abline(h = 1, lty = 3)
axis(side = 1, at = seq(1,22,by=2), labels = rs, las = 1, cex.axis=.75)
mtext(side = 1, "plumage patch", line=3)
par(font=3)
legend('topleft', bty='n', pch=c(21,21,21), 
...
par(mar=c(4,4,2,2))
p <- grep('wing', names(specs.sm))
aggplot(specs.sm[,c(1,p)], by=substr(names(specs.sm)[p], 1, 4), alpha=.3, lwd=2, 
        ylim=c(0,45), lcol=pal[c(3,1,2)], shadecol=pal[c(3,1,2)])
# text(300,40,'wing', pos=4)
par(font=3)
legend('topleft', lty=1, col=pal[c(2,1,3)], 
       legend=c('elisabeth', 'chloropterus', 'acuticaudus'), bty='n')
par(font=1)
...
Finally, to produce a tetrahedral colourspace plot, we used \code{relative=TRUE} in the
\code{vismodel} input to indicate that \{\emph{usml}\} values should be scaled to sum of unity for
each patch. These results indicate that the expansion of occupied colourspace for both subspecies is
largely due to divergence in the colour of the belly patch (see circles labeled `bel' at edges of red
and blue polyhedrons in Figure \ref{figure:volplot}a). Interestingly, the wing plumage patch of
\emph{L. elisabeth} is more  similar to \emph{L. acutacaudus} than \emph{L. chloropterus}, to which
it is more closely related (circles labeled `ch' in Figure \ref{figure:volplot}a).

We also used the \code{voloverlap} function to calculate the volumes occupied by each species'
plumage patches, as well as their overlaps. As an example, the code below shows the result for the
overlap between the two recently diverged species (see Figure \ref{figure:volplot}b):
...
\begin{figure}[H]
\centering
\includegraphics[width=5in]{fig/ms-new_volplot}
\caption{\textbf{a}: Tetrahedral colorspace plot showing minimum convex polygons (convex hulls) containing reflectance spectra for plumage patches from \emph{Lamprotornis acuticaudus} (green), \emph{L. chloropterus} (blue) and \emph{L. elisabeth} (red). Labeled points show representative plumage patches belly (bel) and cheek (ch). \textbf{b}: Colourspace occupied by \emph{L. chloropterus} (blue) and \emph{L. elisabeth} (red). Convex hulls volume overlap (similar colourspace occupied) is shown in grey. For reference, tetrahedron in lower right corner shows \{\textit{usml}\} colour vertices and location of points in colourspace as indicated by an `x'.}
\label{figure:volplot}
\end{figure}

The \code{plot=TRUE} option provides a useful graphical representation of the overlap in colourspace
(Figure \ref{figure:volplot}b). Again, the calculated volume in \pavo{} represents the exact
solution to the geometric problem \citep[i.e. not an approximation obtained through Monte Carlo 
simulation,][]{Stoddard:2011}.
