...
\{\emph{"l"}\}-cone Weber fraction of approximately 0.05 \citep{Vorobyev:1998,Vorobyev:1998to}. The
results of \code{coldist} give colour distances (chromatic, $\Delta S$, shwon in Figure
\ref{figure:distplot}; and achromatic, $\Delta L$) between all possible combinations of
plumage patches in a \code{vismodel} object, and can be further subset (using the \code{subset}
argument) to focus on comparisons of interest (e.g., colour difference between homologous patches of
two species, or between color patches and a given background). This argument employs regular
expressions functionality, and thus allows for partial string matching of row names to the rule
specified in the argument. For example:
