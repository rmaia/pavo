...
from different locations within each patch were collected. We loaded these 396 
...
To quantify spectral reflectance and explore how these colours may be perceived by birds, we used
...
or signal perception mechanisms, see \cite{Montgomerie:2006} and 
\cite{Andersson:2006}. For visualmodels based on photon catches and receptor noise, see 
\cite{Vorobyev:1998} and \cite{Vorobyev:1998to}. For visual phenotype sensitivity curve estimation, see
