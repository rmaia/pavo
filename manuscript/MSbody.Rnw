...
to an increasing interest in modelling species-specific color perception. Despite this growing 
interest, a unified framework for analyzing spectral data is still lacking.
...
data from a variety of sources, implements easy to use visualization tools, and allows the modelling 
of visual systems of any vertebrate and (almost all) invertebrates.
...
aspects of natural selection \citep{Chittka:1992} and sexual selection (REF), and how these interact
\citep{,Kemp:2009}  (REFS- SUGGESTIONS? Endler 1992). It it also an ideal system for truly integrative
biological research, from the optical processes generating color (REF), hormonal and genetic
mechanisms regulating this expression (REF), physiological processes involved in preceiving the
signal (REF), and its adaptive and evolutionary patterns \citep{Badyaev:2003,Darst:2006}.
...
\pavo{} was built on a conceptual tripod of (i) organization and processing of spectral data, (ii)
visualization, and (iii) analysis of the resulting data. The function \code{getspec} handles data
input from a variety of manufacturers (including Avantes, Ocean Optics, CRAIC). Here we demonstrate
some of these capabilities. The data analyzed are reflectance spectra taken from museum specimens of
three species from the monophyletic African starling clade (Sturnidae): \emph{Lamprotornis
elisabeth}, \emph{Lamprotornis chloropterus} and \emph{Lamprotornis acuticaudus}. We measured three
reflectance spectra per plumage patch (n=11, see Figure \ref{figure:distplot} for list) per individual male (n=4).
Recent phylogenetic evidence indicates a very recent divergence for \emph{L. chloropterus} and
\emph{L. elisabeth} (ca. 25 kYa) compared to the node containing \emph{L. acuticaudus} (~XX mYa?).
Given the lability of iridescent plumages (\cite{}) and potential to produce colours across the
bird-visible range of wavelengths, plumage colouration may play a role in reproductive isolation. By
analyzing plumage patches within an avian colourspace (see \cite{}) we can better understand the
divergence in plumage colour among these focal species.
...
% Talk about flexible packages here:

<<label=exploreplot, fig=TRUE, include=FALSE>>=
...
\begin{figure}
\centering
\includegraphics{fig/ms-exploreplot}
\caption{Plots of spectra...} % CHANGE THE PLOTTING METHOD, MAYBE USE AGGPLOT
\label{figure:exploreplot}
\end{figure}

\pavo{} incorporates available data on the retinal sensitivies of numerous taxa, including the
European starling \code{visual = `star'}, which we incorporate here \cite{Hart:1998}. The
\code{relative} argument indicates whether...
...
% tcsplot(tcs1, col = 'red', size=.01)
% tcsvol(tcs1, col='red')
% tcspoints(tcs2, col = 'blue', size=.01)
% tcsvol(tcs2, col='blue')
% tcspoints(tcs3, col = 'green', size=.01)
% tcsvol(tcs3, col='green')
% @

<<label=volplot, fig=T, include=F, width=5, height=5>>=
# Volume overlap calculations
voloverlap(tcs2, tcs3, plot=T)
rgl.postscript('volplot.pdf', fmt='pdf')
@

\begin{figure}
\centering
\includegraphics{volplot}
\caption{Convex hull plot of \emph{Lamprotornis chloropterus} and \emph{L. elisabeth} in avian 
tetrahedral colourspace. Polyhedrons are delimited by plumage patches at the vertices. Colours 
indicate \emph{L. chloropterus} (red) and \emph{L. elisabeth} (blue), with the volume overlap 
(similar colourspace occupied) shown in black.}
\label{figure:volplot}
\end{figure}

Using the \code{voloverlap} function gives a graphical representation of the overlap in colourspace
occupied by each recently diverged species. Moreover, the ability to calculate an exact volume
(rather than an estimated volume derived from Monte Carlo simulation) represents a substantial
advance in the ability to analyze colourspace distributions.
...
delta.S <- coldist(rbind(vm1.rel$Qi, vm2.rel$Qi, vm3.rel$Qi), n1 = 1, n2 = 1.38, 
...
Note that we use \code{relative = FALSE} here to indicate that raw {usml} values should be used,
suitable for calculating chromatic distances \cite{}. Relative UVWS, SWS, MWS and LWS cone densities
taken  from \cite{Hart:1998} and \code{v} was set to 0.085 to give a LWS conde Weber fraction of 0.1
(e.g., see \cite{}).
...
<<label=distplot, fig=TRUE, include=FALSE, width=5, height=5>>=
...
\caption{Dotplot showing colour distances (in units of just noticeable differences, jnds) by patch 
(x-axis) between three pairs of African starling species (Sturnidae:\emph{Lamprotornis}. Dotted 
horizontal lines indicate jnd=1, above which colours are theoretically distinguishable by birds.}
\label{figure:distplot}
\end{figure}

Figure \ref{figure:distplot} shows that nearly all plumage patches are divergent (jnd > 1, dotted
lines in Figure \ref{}) within the subclade as well as compared to the outgroup \emph{L.
acuticaudus}. These data show that the expansion of occupied colourspace for both subspecies (see
Figure \ref{figure:volplot}) is largely due to divergence in the colour of the belly patch (see
Figure \ref{distplot}). Interestingly, the cheek plumage patch of L. elisabeth is more similar to L.
acutacaudus than L. chloropterus (Figure \ref{figure:distplot}).
