...
questions, \textcolor{red}{as well as comparing different approaches to color analysis, all while
integrating with \code{R}'s modular framework to create reproducible and automated data analysis
workflows.} \textcolor{red}{(RM: SHOULD WE CHANGE THIS CLOSING SENTENCE?)}

\end{enumerate} 

\end{abstract}

\keywords{animal communication, colourspace, just noticeable differences, receptor noise, sensory ecology, spectrometry, visual model}
...
referenced by functions downstream in the workflow (e.g. \code{coldist}, below), as well as can be
...
\citep{Andersson:2006} for discussion). For our example, we extracted and plotted an objective
metric of hue (wavelength of peak reflectance, Figure \ref{figure:distplot}C) for the most
