...
\item Animal coloration has been a powerful model for the study of a diverse range of topics in
...
to determine how these colors are perceived by the animals themselves , providing important
...
solution for the calculation of color volume overlap, thus expanding previously published
methodologies.
...
models, we show that color disparity between the recently diverged species is as great as that
...
The role of coloration and color vision in animal communication has been a fundamental question in
...
from the optical processes generating color \citep{Shawkey:2009}, hormonal and genetic mechanisms
...
and processing of color have allowed analysis of  reflectance data using visual models that estimate
how animals see and differentiate these colors
\citep{Goldsmith:1990,Tovee:1995vv,Vorobyev:1998,Vorobyev:1998to}.

because of these advances, a cohesive framework for working with and analyzing color from
...
software, see][]{Montgomerie:2006}. Color data often require additional conversion and export into
...
to intuitively import, explore, process and analyze spectral color data under a variety of user-
...
Additionally, the \code{"tcs"} class refers to tetrahedral colorspace models
\citep{Endler:2005,Stoddard:2008}, and the \code{summary} function can be used to extract summary
variables, like the color volume or hue span \citep{Stoddard:2008} for groups and subsets of points.
...
(cone absorbance), background color, and ambient illuminant \citep{Vorobyev:1998}. Several avian
...
receptors involved in color vision][]{Goldsmith:1990,Endler:2005,Stoddard:2008}. Absolute or
relative cone stimulation can be selected by the logical argument \code{relative} from the
\code{vismodel} function. In the case of the avian tetrahedral colorspace, several additional
...
photon catches before defining the colorspace (by dividing the photon catch of each receptor by the
...
colorspace coordinates (Figure \ref{figure:logplot}, middle). \cite{Stoddard:2008} note that this
...
their colors, and compared both to their sister species, the sharp-tailed starling (\emph{L.
acuticaudus}).
...
compare colors taking into account avian visual sensitivities (sensory phenotype). \pavo{}
...
that color disparity between the closely related species is similar to that between them and the
...
patch comparisons yield colors that are discernible to the starling visual system, both within the
\emph{L. chloropterus-L. elisabeth} subclade as well as compared to the \emph{L. acuticaudus}
outgroup. Given the high satuturation of iridescent colors, small differences in the spectral shape
can yield noticeably different colors. This becomes clear when comparing the results for the most
