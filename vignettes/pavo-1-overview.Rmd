...
knitr::include_graphics("https://raw.githubusercontent.com/rmaia/pavo/master/vignettes/fig/workflow.png")
```

In the included vignettes we begin by detailing the [importing, processing and visualisation](pavo-2-importing_processing.html) of spectral and image data, before moving on to discussion of the [flexible analyses](pavo-3-analysing.html) of such data that `pavo` allows. Our hope is to demonstrate the flexibility of `pavo`, and to provide a cohesive, reproducible workflow for colour pattern analysis within `R`. As always, the development version of `pavo` can be found on [github][pavo_repo], while the stable release is available via CRAN.
...
For suggestions, assistance and/or bug reports, we suggest getting in touch by sending an email to <r-pavo@googlegroups.com>, which is a public mailing list for all things pavo. If you have a bug to report, we'd appreciate it if you could also include a reproducible example when possible. Users familiar with git may prefer to open an issue on the project's [github page][pavo_repo], or to make a pull-request directly.
