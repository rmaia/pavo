...
We detail the changes and additions to the package below, but frequent users should be aware of the following main changes:  
  ...
  * besides the familiar distances in JND for receptor noise models, **`coldist` now also calculates color distances for the other colorspaces available through `colspace`**: TOM BRIEFLY LIST WHICH COLOR DISTANCE METRICS ARE CALCULATED FOR WHICH COLOR SPACES. (also I guess at some point we need to check for consistency of color x colour :P)
...
plot(di.flowers, col = 'forestgreen') 
```
...
In addition, the new `axistetra` function can be used to draw arrows showing the direction and magnitude of distortion of x, y and z in the tetrahedral plot.
...
axistetra(x=1.4, y=0.2, z=-0.4, size=c(0.2,0.2,0.2), label=FALSE)
...
axistetra(x=0.6, y=0.2, z=-0.5, size=c(0.2,0.2,0.2), label=FALSE)
```
