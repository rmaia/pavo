...
\code{data(teal)} will be used in this vignette, but \code{data(sicalis)} is also available. See help 
...
For the chromatic contrast. The achromatic contrast ($\Delta L$) can be calculated based on the double cone or the receptor (or combination of receptors) responsible for chromatic processing by the equation (SIDDIQUI 2004):

\begin{equation}
\Delta L =  \frac{\Delta f^2}{w^2}
\end{equation}

\pavo{} implements these calculations in the function \code{coldist}. For the achromatic contrast, \code{coldist} uses $\frac{v}{\sqrt{n_4}}$ as the value of $w$ for the achromatic contrast. The user can also specify if the function should use $Q_i$, $q_i$ or $f_i$ in the equations (that is, if the von Kries correction and Fechner's law transformation should be applied). Note that even if $Q_i$ or $q_i$ are chosen, values are still log-transformed. This option is available mostly for in case the user wants to specify a data.frame of quantum catches that was not generated by \code{vismodel} as an input.
...
Where \code{dS} is the chromatic contrast ($\Delta S$) and \code{dL} is the achromatic contrast ($\Delta L$). As expected, values are really high, since the colors of these species are quite different (see Figure \ref{figure:usmlplot}). IMPORTANT: Note that \code{coldist} returns $\Delta S$, \textbf{not} $\Delta S^2$
