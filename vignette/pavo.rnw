...
plot(teal.sm[, -14], type='h', varying=angles, 
     ylab="Incident angle (º)", las=1)
@
...
\hline
\code{B1} & {$\sum_{\lambda_{min}}^{\lambda_{max}} R_\lambda$} & \parbox[t]{3in}{Total brightness, total reflectance}  \\
\code{B2} & {$B_{1}/n_{wl}$} & \parbox[t]{3in}{Mean brightness.} \\
\code{B3} & {$R_\text{max}$} & \parbox[t]{3in}{Intensity.} \\
\code{S1} & {$\sum_{\lambda_a}^{\lambda_b} R_\lambda/B_{1}$} & \parbox[t]{3in}{Chroma, spectral purity.} \\
\code{S2} & {$R_\text{max}/R_\text{min}$} & \parbox[t]{3in}{Spectral saturation} \\
\code{S3} & {$\sum_{\lambda_{R\text{max}}-50}^{\lambda_{R\text{max}}+50} R_i/B_{1}$} & \parbox[t]{3in}{} \\
\code{S4} & {$|b\text{max}_\text{neg}|$} & \parbox[t]{3in}{} \\
\code{S5} & {$\sqrt{(B_r-B_g)^2+(B_y-B_b)^2}$} & \parbox[t]{3in}{} \\
\code{S6} & {$R_\text{max}-R_\text{min}$} & \parbox[t]{3in}{} \\
\code{S7} & {$(\sum_{\lambda_1}^{\lambda_R\text{mid}}R_i - \sum_{\lambda_R\text{mid}}^{\lambda_2}R_i)/B_{1}$} & \parbox[t]{3in}{} \\
\code{S8} & {$(R_\text{max}-R_\text{min})/B_2$} & \parbox[t]{3in}{} \\
\code{S9} & {$(R_{\lambda{450}}-R_{\lambda{700}})/R_{\lambda{700}}$} & \parbox[t]{3in}{} \\
\code{S10} & {$[(R_{max}-R_{min})/B_2]\times|b\text{max}_\text{neg}|$} & \parbox[t]{3in}{} \\
\code{H1} & {$\lambda_{R\text{max}}$} & \parbox[t]{3in}{Hue: wavelength of peak reflectance} \\
\code{H2} & {$\lambda_{b\text{max}_\text{neg}}$} & \parbox[t]{3in}{} \\
\code{H3} & {$\lambda_{R\text{mid}}$} & \parbox[t]{3in}{} \\
\code{H4} & {$atan\{[(B_y-B_b)/B_1]/[(B_r-B_g)/B_1]\}$} & \parbox[t]{3in}{} \\
\code{H5} & {$\lambda_{b\text{max}_\text{pos}}$} & \parbox[t]{3in}{} \\
\hline
\end{tabular}
\end{center}
