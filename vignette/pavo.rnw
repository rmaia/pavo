...
use local regression smooth implemented by the \code{loess.smooth} function in \code{R}. However, we
will first use the \code{plotsmooth} function to determine a suitable smoothing parameter
...
In addition to smoothing, we can also try some other normalizations implemented in \code{procspec}.
Note that the user can specify multiple processing  options that will be applied sequentially to the
spectral data by \code{procspec}.
...
vismod1 <- vismodel(sppspec, visual = "avg.uv")
