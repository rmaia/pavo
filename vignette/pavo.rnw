...
(for example, using Principal Component Analysis, or PCA). Following Cuthill \cite{Cuthill:1999}, 
we can use 
...
directly  use sensory information; the model of Osorio \& Vorobyev \cite{Vorobyev:1998}, which
...
\textbf{Color distances}. Under the colorspace framework, color distances can be calculated simply as 
