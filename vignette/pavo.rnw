...
{$H_4 = atan\{[(B_y-B_b)/B_1]/[(B_r-B_g)/B_1]\}$} & \parbox[t]{3in}{\textbf{Hue}: the angle from 0\degree (red) calculated by the segment classification method (see section \ref{sec:segclass}).} \\
