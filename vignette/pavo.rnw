...
\end{figure}
 ...
 \subsubsection{Processing Spectral Data}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=5in]{pavo-specsmooth}
\end{center}
...
<<Label=specnorm,fig=TRUE,include=FALSE>>=
# plot results
par(mfrow=c(1,3), mar=c(4,4,1,1))
plot(specs.max, select=5, ylim=c(0,1))
abline(h=1, lty=2)
plot(specs.min, select=5)
abline(h=0, lty=2)
plot(specs.str, select=5)
abline(h=c(0,1), lty=2)
@

\begin{figure}[H]
\begin{center}
\includegraphics[width=8, height=3]{pavo-specnorm}
\end{center}
...
This function relies on the base plotting capabilities of \code{R} but recognizes \code{rspec}
objects.

<<label=pcaplot,fig=TRUE,include=FALSE>>=
...
# plot results
par(mfrow=c(1,2), ps=8, mar=c(4,4,2,2))
plot(pca1$r[,1]~wls, type='l', xlab="Wavelength (nm)", ylab="PC1 loading")
abline(h=0, lty=2)
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=6.3in, height=3.3in]{pavo-pcaplot}
\end{center}
...
par(mar=c(4,4,2,2))
plot(sppspec, type='o', col=spec2rgb(sppspec))
plot(sppspec, type='s', col=spec2rgb(sppspec))
#plot(sppspecs, type='h', col=spec2rgb(sicalis))
@

\begin{figure}[H]
\begin{center}
\includegraphics[width=4]{pavo-overlay}
\end{center}
...
<<label=segplot, fig=TRUE, include=FALSE, width=2>>=
fakedata1<-  sapply(seq(100,500,by=20), function(x) rowSums(cbind(dnorm(300:700,x,30), dnorm(300:700,x+400,30))))
...
<<label=segplot2, echo=FALSE, fig=TRUE, include=FALSE>>=
plot(segfd1, pch=20, cex=2, col=spec2rgb(fakedata1))
@
...
\centering
  \begin{subfigure}[b]{0.2\textwidth}
    \includegraphics{pavo-segplot}
  \end{subfigure}
  \begin{subfigure}[b]{0.6\textwidth}
    \includegraphics{pavo-segplot2}
  \end{subfigure}
\caption{segment plot}\label{fig:segplot}
\end{figure}
