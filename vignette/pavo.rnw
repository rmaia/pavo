...
will make the cone stimulations relative to their sum. This will be useful in the following sections, 
\textit{but for the photon catch and neural noise model, it is important to set} \code{relative=FALSE}.
...
for (i in 1:6) {
  par(mar=c(2,2,2,2))
  plot(sppspec, select = i + 1, col = spec2rgb(sppspec), lwd = 3, ylim = c(0,100))
