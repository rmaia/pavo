...
plotsmooth(teal, minsmooth = 0.05, maxsmooth = 0.25, curves = 4, specnum = 6, ask = FALSE)

@

\begin{figure}[H]
\begin{center}
\includegraphics[width=3in]{fig/pavo-plotsmooth}
\end{center}
...
we will first load some reflectance data, included as the \code{teal} dataset in \pavo{}. 
...
(in 5\ensuremath{^\circ} increments) (CHAD DUCK PAPER REF). 
