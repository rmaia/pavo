...
The data used in this example is available from \href{http://github.com}github by clicking 
...
times, as is common in order to avoid measurement bias. A good way to visualize the  repeatability
...
We can also try some different normalizations. Options include subtracting the minimum reflectance
...
by adding the minimum reflectance to all other wavelngth, the shape of the curve is preserved, but
...
\textbf{Receptor Noise.} Color distances can be calculated under this model considering receptor
...
and the relative number of receptor  cells of type $i$ within the recptor field ($n_i$):
...
\textbf{Plotting options}. There are two useful plots for the tetrahedral colorspace model. The
