...
\begin{equation}
k_i = 1/\int_\lambda{R_i(\lambda)S^b(\lambda)I(\lambda)d\lambda}, 
\end{equation}
...
\begin{equation}
\begin{aligned}
\Delta S^2 = \frac{(\Delta f_1 - \Delta f_2)^2}{w_1^2+w_2^2}
\end{aligned}
\end{equation}
...
So it can be clearly seen that there is overlap between the throat and breast colors, but not between 
