...
Negative values in spectra are unwanted, as they can affect estimates of color variables. Certain
spectrometer manufacturers allow negative values to be saved. To handle negative values, the
\code{procspec} function has an option called \code{fixneg}. The two options available are (1)
adding the absolute value of the most negative value to the whole spectrum (\code{fixneg = addmin})
and (2) changing all negative values to zero (\code{fixneg = zero}).

<<results=hide>>=
# Create a duplicate spectrum and add some negative values
refl <- sppspec[, 7]
samp <- sample(1:401, 25)
refl[samp] <- refl[samp] - 20
...
plot(testspecs, select = 2, ylim=c(-10,40))
abline(h=0, lty=3)
plot(testspecs.fix1, select = 2, ylim=c(-10,40))
abline(h=0, lty=3)
plot(testspecs.fix2, select = 2, ylim=c(-10,40))
abline(h=0, lty=3)
mtext("Wavelength (nm)", side=1, outer=T, line=1)
mtext("Reflectance (%)", side=2, outer=T, line=1)
@
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=4in]{pavo-fixneg}
\end{center}
