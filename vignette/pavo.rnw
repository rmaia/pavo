...
Obtaining colorimetric variables (peratining to hue, saturation and brightness/value) is pretty
...
Montgomerie \cite{Montgomerie:2006}. Though outlined in a book chapter on bird coloration, these
variables are broadly applicable to any reflectance data, particularly if the taxon of interest has
color vision within the UV-human visible range.\code{summary} also takes an additional argument
\code{simplify} which is changed from the default \code{FALSE} to \code\{TRUE} will return only
...
recommended by Stoddard \& Prum \cite{Stoddard:2008}), but $q_i$ or $f_i$ may be specified if
desired
