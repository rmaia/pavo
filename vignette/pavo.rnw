...
for working with spectral color data. It includes functions that take advantage of new object classes to work seamlessly from importing raw data to visualization and analysis.

Although \pavo{} deals largely, in its examples, with spectral reflectance data from bird feathers, it is meant  to be
...
Data obtained from spectrometers often require further processing before analysis and/or
...
(for example, using Principal Component Analysis, or PCA). Following Cuthill et al. (1999), we can use 
...
(1990) segment classification method, which approximates visual models but does not directly  use
sensory information; the model of Vorobyev \& Osorio (1998), which provides a flexible  framework for
visual modeling; and the tetrahedral color space (Goldsmith 1990, Endler et al. 2005, Stoddard and 
Prum 2008)  which has been
...
as outlined in Montgomerie(2006). Though outlined in a book chapter on bird coloration,  
these variables
...
individual's or species' receptor visual system (Described in detail in Endler 1990, Vorobyev 
\& Osorio 1998). In essence,
...
recommended by Stoddard \& Prum 2008), but $q_i$ or $f_i$ may be specified if desired 
(e.g. \code{tcs(vismod2\$fi)}).


\code{tcs} returns the original photon catch values; the relative cone stimulation for a given hue
(\code{x.r}; see the supplemental material of Stoddard \& Prum 2008 for more information); the 
two angles
...
dataset, which includes measurements from the crown (C),  throat (T) and belly (B) of seven stripe-
...
Cuthill IC, Bennett ATD, Partridge JC, \& EJ Maier (1999) Plumage reflectance and the objective
assessment of avian sexual dichromatism. American Naturalist 153:183-200.

Eliason CM \& MD Shawkey (2012) A photonic heterostructure prouces diverse iridescent colours in duck
wing patches. Journal of the Royal Society Interface 9:2279-2289.

Endler JA (1990) On the measurement and classification of color in studies of animal color patterns.
Biological Journal of the Linnean Society 41:315-352.

Endler JA, Wescott DA, Madden JR, \& T Tobson (2005) Animal visual systems and the evolution of color
patterns: sensory processing illuminates signal evolution. Evolution 59:1795-1818.

Goldsmith TH (1990) Optimization, constraint, and history in the evolution of eyes. Quarterly Review of
Biology 65:281-322.

Govardovskii VI, Fyhrquist N, Reuter T, Kuzmin DG, \& K Donner (2000) In search of the visual pigment
template. Visual Neuroscience 17:509-528.

Hart NS, \& M Vorobyev (2005) Modelling oil droplet absorption spectra and spectral sensitivities of
bird cone photoreceptors. Journal of Comparative Physiology A 191:381-392.

Montgomerie R (2006) Analyzing colors. Pages 90-147 in G. E. Hill and K. J. McGraw, eds. Bird 
coloration. Vol. 1. Mechanisms and measurements. Harvard University Press, Cambridge, MA.

Stoddard MC \& RO Prum (2008) Evolution of avian plumage color in a tetrahedral color space: 
A phylogenetic analysis of new world buntings. American Naturalist 171:75-776.

Vorobyev M \& D Osorio (1998) Receptor noise as a determinant of colour thresholds. Proceedings of 
