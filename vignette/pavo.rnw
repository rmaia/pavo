...
\title{\pavo{}: An R {\bf P}ackage for the {\bf A}nalysis, {\bf V}isualization and {\bf O}rganization of Spectral Data}
...
\item {\bf Analyze} the resulting files, either using typical colorimetric variables (hue,
...
\code{data(teal)} and \code{data(sicalis)} will both be used in this vignette. See
...
<<echo=TRUE, eval=FALSE, results=hide, include=FALSE>>=
specs <- getspec("~/github/pavo/vignette_data/", ext="ttt", decimal=",", 
                 subdir=T, subdir.names=F)
...
with the \code{whichwl} argument. The default way that \code{as.rspec} handles reflectance data is
to interpolate the data in 1-nm bins, as is commonly done for spectral analyses. However, this can
be turned off by using: \code{interp = FALSE}. As an example, we will create some fake reflectance 
data, name the column containing wavelengths (in 0.5-nm bins) ``wavelength'' rather than ``wl'' 
...
As can be seen, \code{as.rspec} renames the column containing wavelengths, sets it as the first 
column, interpolates the data in 1-nm bins and converts the data to an \code{rspec} object. Note that
the same output is returned with specifying \code{whichwl = 3}:

<<>>=
head(as.rspec(fakedat, whichwl = 3))
@

Finally, the \code{lim} argument allows you to specify the range of wavelengths contained in the
input dataset. This is useful either in the case that the dataset doesn't contain this information 
(and hence you cannot specify the column with \code{whichwl} or automatically find the column with
\code{as.rspec}). Additionally, it may be useful to focus on a subset of wavelength. In our example, 
the wavelengths ranged from 300 to 700 nm, however you could also specify a restricted range of 
wavelengths with \code{lim}:
...
setting this argument (\code{as.rspec} will warn you when wavelengths in the data are not present in
the range specified with \code{lim}), otherwise \code{as.rspec} will assume that wavelengths exist 
when in fact they may not. For example, if we set \code{lim = c(300, 1000)} and plot the results, the 
...
% TODO: do we need this plot? if so, i need to set the plot size. also, maybe it would be better
% to use actual reflectance data?
...
Once an \code{rspec} object has been created, either by importing raw spectral data or converting
a dataset with the \code{as.rspec} function, you can subset the spectra based on their names using 
a modified version of \code{R}'s built-in \code{subset} function. For example, the following code
illustrates how to create an \code{rspec} object containing only tanagers:
...
The \code{subset} function here is using partial matching to find all spectra with the string
``tanager'' in their name. To fully benefit from this flexible subsetting functionality, it is
important that you follow a consistent file naming scheme. For example,
``\code{tanager.14423.belly.001.ttt}'' would indicate the species (``tanager''), individual ID
(14423), body patch (belly) and measurement number (001). Additionally, we suggest that labels used
should have the same number of characters, which simplifies character string manipulation and
subsetting based on partial matching. Please see Andersson \& Prager \citep{Andersson:2006} for a 
useful discussion and suggestions on file naming.

If you prefer not to use partial matching, \code{subset} will also work if you provide a logical 
condition, similar to the default \code{subset} behavior in \code{R}. For example:

<<>>=
spp <- do.call(rbind, strsplit(names(specs), "\\."))[,1]  # vector of first component of filenames
specs.tanager2 <- subset(specs, subset = spp=="tanager")
all.equal(specs.tanager1, specs.tanager2)
@

On a final note, \code{subset} will also work with visual model (class \code{vismodel}) and 
tetracolorspace (class \code{tcs}) objects, as described below.
...
As previously described, our data (contained in the \code{specs} object) constitutes of multiple 
...
The function \code{explorespec} provides an easy way of doing so. You may specify the number of  
spectra to be plotted in the same panel using the argument \code{specreps}, and the function will 
...
Instead, we are going to use the \code{spp} vector we created to tell the \code{aggspec} function how 
to average the spectra in \code{mspec}:
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=2.5in]{fig/pavo-exploresppmeans}
\end{center}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=2.5in]{fig/pavo-specsmooth}
\end{center}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=5in]{fig/pavo-specnorm}
\end{center}
...
(for example, using Principal Component Analysis, or PCA). Following Cuthill et al. 
\cite{Cuthill:1999}, we can use \code{opt = `center'} to center spectra to have a mean reflectance of 
zero (thus removing brightness as a dominant variable in the PCA) and then bin the spectra into user
-defined bins (using the \code{opt = `bin'} argument) to obtain a dataframe suitable for the PCA.
...
\code{plot} function. % CE: should we delete this part?
...
\textcolor{red}{RM: THIS IS THROWING AN ERROR BECAUSE REFL IS A VECTOR, NOT A DATA.FRAME, 
AND THUS DOESNT  HAVE COLUMNS. AS.RSPEC HAS A 1:NCOL(OBJECT) CALL IN IT. I DID A WORKAROUND
BY SELECTING TWO COLUMNS INSTEAD OF ONE, BUT WE SHOULD FIX THIS.}
...
% LEFT OFF HERE -- ERROR IN FUNCTION
...
These manipultions may have different effects on the final spectra, as can be seen in Figure
\ref{figure:neg}, which the user should keep in mind and use according to the final goal of the
analysis. For example, by adding the minimum reflectance to all other wavelength, the shape of the
curve is preserved, but the maximum reflectance is much higher (Figure \ref{figure:neg}, top). On
the other hand, substituting negative values with zero preserves absolute reflectance values, but
may cause the spectral shape to be lost (Figure \ref{figure:neg}, bottom). The ``best''
...
spectrophotometer, \code{fixneg = addmin} would be appropriate. However, if they are thought to
originate from electric noise, \code{fixneg = zero} would be more appropriate.
...
increments) (Eliason \& Shawkey, 2012).
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=5in]{fig/pavo-aggplot}
\end{center}
...
spectra data frame objects, this can easily be done using the \code{summary} function with an
\code{rspec} object (see below). The function \code{peakshape} also returns descriptors for
...
\cite{Endler:1990} segment classification method, which approximates visual models but does not
directly  use sensory information; the model of Vorobyev \& Osorio \cite{Vorobyev:1998}, which
provides a flexible  framework for visual modeling; and the tetrahedral color space
\cite{Goldsmith:1990, Endler:1990, Stoddard:2008} which has been extensively developed to represent
colors in the avian vision color space.


\subsection{Spectral Shape Analysis}

\subsubsection{Colorimetric Variables}
...
as outlined in Montgomerie \citep{Montgomerie:2006}. Though outlined in a book chapter on bird
...
\code{summary} also takes an additional argument \code{subset} which if changed from the 
default \code{FALSE} to \code{TRUE} will return only the most commonly used colorimetrics 
\citep{Andersson:2006}. \code{summary} can also take a vector of color variable names, which
...
the  Tanager (Figure \ref{figure:fwhm}, last panel). In this case, it's useful to set wavelength 
limits when calculating the FWHM by using the \code{lim} argument. \code{peakshape} also offers a
...
peakshape(spec.sm, select=2, lim=c(300,500), plot=T)
@
\begin{figure}[H] % h=here, t=top, b=bottom, p=floating page
\begin{center}
\includegraphics[width=3in]{fig/pavo-fwhm2}
\end{center}
\caption{Plot from \code{peakshape}, setting the wavelength limits to 300 and 500nm}
\label{figure:fwhm2}
\end{figure}
...
The segment classification analysis \cite{Endler:1990} does not assume any particular visual system,
...
individual's or species' receptor visual system (Described in detail in \cite{Endler:1990,
Vorobyev:1998}). In essence, these models take into  account the receptor sensitivity of the
...
\textbf{Photon Catch.} To quantify the stimulation of cones by the emitted color, we will use the \pavo{} function \code{vismodel}. This function takes an \code{rspec} dataframe as a minimal input, 
and the user can either select from the available options or input its own data for the additional
...
The \code{vismodel} function also takes additional arguments that can be used to customize the visual
model being implemented:

\begin{itemize}
  \item\code{qcatch}: This argument determines what photon catch data should be returned. 
    \begin{itemize}
    ...
    \end{itemize} 
  \item\code{relative}: If \code{TRUE}, it will make the cone stimulations relative to their sum. 
  This is appropriate for colorspace models such as the avian tetrahedral colorspace
  \citep{Goldsmith:1990, Stoddard:2008} \textit{For the photon catch and neural noise 
  model, it is important to set} \code{relative=FALSE}.
  \item\code{vonkries}: a logical argument which determines if the von Kries transformation (which
  normalizes receptor quantum catches to the background, thus accounting for receptor adaptation)
  is to be applied (defaults to \code{FALSE}). If \code{TRUE}, $Q_i$ is multiplied by a constant $k$,
  which describes the von Kries transformation:
  \begin{equation}
  ...
  \item\code{scale}: This argument defines how the illuminant should be scaled. The scale of the
  illuminant is critical of receptor noise models in which the signal intensity influences the noise
  (see Receptor noise section, below).Illuminant curves should be in units of $\mu mol.s^{-1}.m^{-2}$ 
  in order to yield physiologically meaningful results. \footnote{some software return illuminant
  information values in $\mu Watt.cm^{-2}$, and must be converted to $\mu mol.s^{-1}.m^{-2}$. 
  This can be done by multiplying the illuminant by 11964.7. For more information, see \cite{Avicol}.} 
  Therefore, if the user-specified illuminant
  curves are \textit{not} in these units (i.e. are measured proportional to a white standard, for
  example), the \code{scale} parameter can be used as a multiplier to yield curves that are at least
  a reasonable approximation of the illuminant value. Commonly used values are \textbf{500} for dim 
  conditions and \textbf{10,000} for bright conditions.
\end{itemize}
...
illumination, and an idealized background.\footnote{up to version 0.1-2, \code{vismodel} would 
return a list containing $Q_i$, $q_i$ and $f_i$. Users should note that this has changed in newer
versions. $Q_i$ is the default value returned, and $f_i$ can be chosen using the \code{qcatch}
argument. $q_i$ can be returned by using the new argument \code{vonkries = TRUE}.} Following 
Vorobyev 1998\cite{Vorobyev:1998}, spectra are converted to proportions (instead of percent 
reflectance) automatically by the function prior to calculations:
...
Since there are multiple parameters that can be used to customize the output of \code{vismodel}, 
for convenience these can be returned by using \code{summary} in a \code{vismodel} object:
...
\begin{figure}[t] % h=here, t=top, b=bottom, p=floating page
\begin{center}
...
et al. \cite{Govardovskii:2000} and Hart \& Vorobyev \cite{Hart:2005}. \code{sensmodel} takes
several optional arguments, but the  main one is a vector containing the peak sensitivities for the
cones being modeled. Let's model an  idealized dichromat visual system, with cones peaking in
sensitivity at 350 and 650 nm:
...
\code{coldist} uses \code{n4} to calculate $w$ for the achromatic contrast. Note that even if $Q_i$ 
is chosen, values are still log-transformed. This option is available in case the user wants to 
specify a data frame of quantum catches that was not generated by \code{vismodel} as an 
input. In this case, the argument \code{qcatch} should be used to inform the function if $Q_i$ or $f_i$
values are being used (\textit{note that if the imput to \code{coldist} is an object generated using
the \code{vismodel} function, this argument is ignored.}) The type of noise to be calculated can be
selected from the \code{coldist} argument \code{noise} (which accepts either \code{"neural"} or
\code{"quantum"}).
...
these species are quite different (see Figure \ref{figure:usmlplot}) and because of the enhanced
discriminatory ability with four compared to two cones.

\code{coldist} also has a \code{subset} argument, which is useful if only certain comparisons are of 
interest (for example, of color patches against a background, or only comparisons among a species or
body patch). \code{subset} can be a vector of length one or two. If only one subsetting option is 
...
particular cone type (Figure \ref{fig:tcs}).
...
(\code{x.r}; see the supplemental material of Stoddard \& Prum \cite{Stoddard:2008} for more
information); the two angles of  hue (\code{h.theta} and \code{h.phi}) and the distance from the
achromatic center (\code{r.vec});  along with the maximum distance achievable for that hue
(\code{r.max}) and the proportion of that  maximum achieved by the color point (\code{r.achieved}).

\textbf{Color distances}. Under the color space framework, color distances can be calculated simply
...
\code{R} using the  \code{dist} function on the \code{vismodel} output (excluding the luminance
data) or the \code{tcs}  output by selecting the cone stimulation data:
...
plot that the user can spin around and zoom, and can be called by the function \code{tcsplot}:
...
The accessory functions \code{tcspoints} and \code{tcsvol} can be used, in addition, to plot
...
\begin{figure}[H]
        ...
        \begin{subfigure}[b]{0.45\textwidth}
                ...
                \label{fig:tcs2}
        \end{subfigure}
         \caption{Example plots obtained using \code{tcsplot}. Plot on the left was exported as pdf, while the one on the right was exported as png (\code{tcsplot} uses the \code{rgl} package for interactive 3D plotting capabilities, and \code{rgl} does not currently support transparency when exporting as \code{pdf}).}\label{fig:tcs}
\end{figure}
...
\begin{figure}[H] % h=here, t=top, b=bottom, p=floating page
\begin{center}
\includegraphics[width=3in]{fig/pavo-proj}
\end{center}
...
overlapping) ``sensory niches'', or to test for mimetism, dichromatism, etc. \cite{Stoddard:2011}.
To show this function, we will use the \code{sicalis} dataset, which includes measurements from
the crown (C), throat (T) and breast (B) of seven stripe-tailed yellow finches (\textit{Sicalis
citrina}).
...
\begin{figure}[b] % h=here, t=top, b=bottom, p=floating page
\begin{center}
\includegraphics[width=2in]{fig/pavo-sicalis}
\end{center}
...
\begin{figure}[H]
        ...
        \begin{subfigure}[b]{0.45\textwidth}
                ...
                \label{fig:ov2}
        \end{subfigure}
         \caption{Color volume overlaps. Shaded area in panel a represents the overlap between those two sets of points.}\label{figure:overlap}
\end{figure}
...
workflow of spectral data color analysis \textit{within} \code{R}, without the need for additional 
...
through the \code{sensmodel} function.
...
within our visual model functions, contact us via email 
(\href{mailto:rm72@zips.uakron.edu}{rm72@zips.uakron.edu}).
...
We would like to thank Matthew D. Shawkey and St\'{e}phanie M. Doucet for insights and support, and 
...
{$S_{10} = [(R_{max}-R_{min})/B_2]\times|b\text{max}_\text{neg}|$} & \parbox[t]{3in}{\textbf{Peaky chroma}: relative contrast (S8) multiplied by the spectral purity (S4). Relatively flat curves will give low values for this metric, and vice versa.} \\
...
The complete set ofcolorimetric variables calculated by \code{summary} in \pavo{} (adapted from
Montgomerie 2006 \cite{Montgomerie:2006})}
\end{table}
