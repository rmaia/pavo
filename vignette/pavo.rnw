...
\subsection{Subsetting and Merging Data}
...
``\code{tanager.14423.belly.001.ttt}'' would indicate the species (tanager), individual ID (14423), 
...
Note that \code{subset} will also work with visual model (class \code{vismodel}) and 
tetracolorspace (class \code{tcs}) objects, as described below.

Another useful function is \code{merge}. Let's say that you have subsetted spectra for tanagers and
...
Note that this re-combined file (\code{specs.new}) has only a single `wl' column with the merged
spectra as columns. Keep in mind that the order of objects in \code{merge} will determine the order
...
\begin{itemize}
  ...
  This can be done by using the \code{irrad2flux} and \code{flux2irrad} functions. For more 
  information, see \cite{Avicol}.} 
...
\begin{equation}
\Delta L =  \left|\frac{\Delta f}{w}\right|
\end{equation}
...
\cite{Stoddard:2008}, and for colour volume overlap see \cite{Stoddard:2008} and
\cite{Stoddard:2011}. Users of the functions that apply these methods must cite the original sources
