...
AND THUS DOESN'T  HAVE COLUMNS. AS.RSPEC HAS A 1:NCOL(OBJECT) CALL IN IT. I DID A WORKAROUND
...
\subsubsection{Colorimetricc Variables}
...
as outlined in Montgomerie \cite{Montgomerie:2006}. Though outlined in a book chapter on bird
coloration, these variables are broadly applicable to any reflectance data, particularly if the
taxon of interest has color vision within the UV-human visible range.\code{summary} also takes an additional
argument \code{simplify} which is changed from the default \code{FALSE} to \code\{TRUE} will return only  
the most commonly used colorimetrics \cite{Andersson:2006}.
...
\textcolor{red}{RM ADD INFO ON QI/FI RETURN}
...
\textcolor{red}{CHANGE THIS, ADD FOOTNOTE ABOUT CHANGE}

\textcolor{red}{ADD INFO ON VONKRIES, SCALE, AND ALL NEW PARAMETERS}
...
\textcolor{red}{RM COLDIST DOESNT NEED QCATCH ANYMORE IF VISMODEL}
...
recommended by Stoddard \& Prum \cite{Stoddard:2008}), but $q_i$ or $f_i$ may be specified if desired 
