...
Colors can be mapped to spectra using \code{spec2rgb} as shown in Figure \ref{figure:stack}.
...
{$S_2 = R_\text{max}/R_\text{min}$} & \parbox[t]{3in}{Spectral saturation: ratio of maximum and miminmum reflectance values.} \\
...
{$S_7 = (\sum_{\lambda_1}^{\lambda_R\text{mid}}R_i - \sum_{\lambda_R\text{mid}}^{\lambda_2}R_i)/B_{1}$} & \parbox[t]{3in}{???} \\
...
{$S_9 = (R_{\lambda{450}}-R_{\lambda{700}})/R_{\lambda{700}}$} & \parbox[t]{3in}{Carotenoid chroma: ...} \\
{$S_10 = [(R_{max}-R_{min})/B_2]\times|b\text{max}_\text{neg}|$} & \parbox[t]{3in}{Peaky chroma: relative contrast (S8) multiplied by the spectral purity (S4). Relatively flat curves will give low vales for this metric, and vice versa.} \\
...
{$H_4 = atan\{[(B_y-B_b)/B_1]/[(B_r-B_g)/B_1]\}$} & \parbox[t]{3in}{Hue: the angle from 0º (red) calculated by the segment classification method.} \\
